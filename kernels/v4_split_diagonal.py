"""
OptiKernel opus_v3 — CSR-only block-sparse causal attention for H100.

Single CSR kernel handles all 3 families (window, global, retrieval).
...
# ═══════════════════════════════════════════════════════════════════
# CSR kernel: handles all sparsity families via CSR col_idx
# ═══════════════════════════════════════════════════════════════════
@triton.jit
def _fwd_csr(
    Q, K, V, Out, LSE,
    ...
    for local_qb in range(QBLOCKS_PER_PROG):
        ...
        if q_start >= seq_len or qb >= num_q_blocks:
            # Write zeros/inf for fully out-of-range blocks
            ...
            # KV positions provably within seq_len (k_end <= q_start < seq_len)
